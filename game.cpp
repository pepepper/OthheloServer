...
Game::Game(SOCKET connection, int w, int h): host(connection), islocked(0), guest(INVALID_SOCKET), x(w), y(h){
	std::chrono::system_clock::time_point start = std::chrono::system_clock::now();
	room = std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(start.time_since_epoch()).count());
	std::string reply = "SUCCESS " + room + "\0";
	send(host, reply.c_str(), reply.length(), 0);
	FD_ZERO(&socks);
	FD_SET(host, &socks);
	thread = std::thread([this]{
		...
		while(1){
			...
			if(FD_ISSET(host, &fds)){
				if(recv(host, data, 32, 0) == -1)break;
				req += data;
				send(guest, req.c_str(), req.length(), 0);
			}

			if(FD_ISSET(guest, &fds)){
				if(recv(guest, data, 32, 0) == -1)break;
				req += data;
				send(host, req.c_str(), req.length(), 0);
			}
			if(!req.compare("CLOSED"))break;
...
#endif
void Game::setPassword(std::string pass){
	islocked = 1;
	password = pass;
...
void Game::readyGame(){
	...
	send(host, reply.c_str(), reply.length(), 0);
	send(guest, reply.c_str(), reply.length(), 0);
	return;
}
